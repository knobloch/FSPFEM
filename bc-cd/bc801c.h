...
#define CC0   1.   /*   min( react(x) - 0.5*div_bb(x) ); if min < 0, CC0=0  */
 ...
 double x[2];
 {
    return( (2.*x[0]-1.)*(2.*x[0]-1.)*(2.*x[0]-1.)*(1.-exp_f(x))/(1.-exp(-2./VALUE_OF_EPS)));
 }
 
 double t01(x)
 double x[2];
 {
    return( 6.*(2.*x[0]-1.)*(2.*x[0]-1.)*(1.-exp_f(x))/(1.-exp(-2./VALUE_OF_EPS)));
 }
 
 double t02(x)
 double x[2];
 {
    return( -(2.*x[0]-1.)*(2.*x[0]-1.)*(2.*x[0]-1.)*exp_f(x)/(1.-exp(-2./VALUE_OF_EPS))*2./VALUE_OF_EPS);
 }
 ...
 double x[2];
 {
    return( -VALUE_OF_EPS*24.*(2.*x[0]-1.)*(1.-exp_f(x))/(1.-exp(-2./VALUE_OF_EPS))
            -2.*t02(x) + bb0(x)*t01(x) + bb1(x)*t02(x) + react(x)*t0(x) );
 }
